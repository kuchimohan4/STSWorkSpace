...
import com.jfs.security2.filter.CsrfCookieFilter;
...
public class ProjectSecurityConfig {
	...
	SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		...
		http.securityContext().requireExplicitSave(false).and()
				...
				.requestMatchers("/myAccount").hasAuthority("USER").requestMatchers("/myBalance")
				.hasAnyAuthority("USER", "ADMIN").requestMatchers("myLoans").hasAuthority("MANAGER")
				.requestMatchers("/myCards").hasAuthority("VIEWCARD").requestMatchers("/user").authenticated()
